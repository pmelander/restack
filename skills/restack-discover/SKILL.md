...
**Discovery output feeds `/restack-stressor walk` directly.** You cannot walk a path
...
## `/restack-discover actor <actor-name>`
...
These go to `/restack-stressor generate` alongside technical stressors and get scored
...
Not ready routes to a **specific** command — `/restack-discover actor <name>`,
`/restack-discover intentions`, `/restack-discover paths`, `/restack-discover organisation` — never to
"do more discovery".
...
```
1. /restack-discover paths          map what is there; settle the boundary
2. /restack-discover actor <name>   investigate critical or opaque actors
3. /restack-discover intentions     trace key intentions end to end
4. /restack-discover gaps           prioritise what is still unknown
5. /restack-discover organisation   map resistance as stressors
6. /restack-discover confidence     CONFIDENCE GATE - not ready routes back to 1-5
   |
   v  ready
7. /restack-stressor walk           walk the discovered paths
```
...
**When change is blocked:** go straight to `/restack-discover organisation`, translate
...
| Skill | How |
|---|---|
| `/restack-journey` | terrain classification sets this skill's confidence threshold |
| `/restack-stressor walk` | discovered paths are its input |
| `/restack-stressor generate` | organisational stressors are injected here |
| `/restack-adr create` | a significant discovery that shapes the approach is a decision |
| `/restack-solution-doc` | discovery output becomes the baseline architecture description |
| `/restack-design-review` | discovery findings make the review honest about assumptions |
