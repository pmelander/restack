...
| Trigger | Run | Because |
|---|---|---|
| A significant change ships | `/discover paths` | the path map may no longer be true |
| Paths changed | `/stressor walk` on affected paths only | re-walk what moved, not everything |
| An incident occurred | `/stressor generate`, then `/arch-learning analyze` | an incident is a stressor that found you first |
| A new integration or dependency | `/discover actor` | a new actor is a new column in the matrix |
| A regulation or contract changes | `/stressor compliance <pack>` | new stressor class, same loop |
| A residual is implemented | `/journey iterate` | confirm impact actually fell |
...
|---|---|---|
| Monthly | `/evolve health` | fitness function drift |
| Quarterly | `/stressor generate` | new stressor classes in the environment |
| Quarterly | `/design-review complete` | architectural drift from the documented target |
| Quarterly | `/arch-learning analyze` | which past decisions actually played out well |
| Quarterly | `/patterns evolve` | promote what worked into the pattern library |
| Every 6 months | `/capability-assessor assess` | team growth against the architecture's demands |
| Continuously | `/adr create` | every significant decision, as it is made |
